...
![Смена пути по умолчанию для установки VM](image/img02.png){#fig:002 width=70%}
...
```
![Проверка поставленных настроек](image/img03.png){#fig:003 width=70%}
...
![Начальные настройки ВМ](image/img07.png){#fig:007 width=70%}

Далее нужно задать размер оперативной памяти, а также количество процессоров:
![Дополнительные настройки ВМ](image/img08.png){#fig:008 width=70%}

Также нужно поставить галочку напротив пункта “Включить UEFI”:
![Включение поддержки UEFI](image/img09.png){#fig:009 width=70%}

Последним шагом будет выделение места на диске под виртуальную машину:
![Выделение места на диске согласно условиям лабораторной работы](image/img10.png){#fig:010 width=70%}
...
![Добавление IDE и SATA контроллеров](image/img11.png){#fig:011 width=70%}

Включим 3D-ускорение и поставим использоваться VMSVGA графический контроллер:
![Включение графических настроек](image/img12.png){#fig:012 width=70%}

Далее включим общий буфер обмена и функцию Drag’n’ Drop:
![Включение буфера обмена и функции Drag’n’ Drop](image/img13.png){#fig:013 width=70%}

## 4. Установка операционной системы 
После запуска ВМ появляется меню запуска. В нем выбираем полноценный запуск Fedora:
![Меню запуска](image/img14.png){#fig:014 width=70%}
...
![Anaconda и настройка ОС](image/img15.png){#fig:015 width=70%}
...
![Настройки часового пояса, а также даты и времени](image/img16.png){#fig:016 width=70%}

Настроим аккаунт администратора (`root`):
![Настройка root пользователя](image/img17.png){#fig:017 width=70%}

Настроим аккаунт пользователя:
![Настройка обычного пользователя](image/img18.png){#fig:018 width=70%}

Настроим сетевое имя ВМ:
![Сетевое имя ВМ](image/img19.png){#fig:019 width=70%}
...
![Вход под ролью суперпользователя](image/img20.png){#fig:020 width=70%}

Обновляем все пакеты командой `dnf -y update`:
![Обновление пакетов](image/img21.png){#fig:021 width=70%}

После этого производим установку `tmux` и `mc`:
![Установка tmux и mc](image/img22.png){#fig:022 width=70%}

## 5. Автоматическое обновление 
Используя команду `dnf install dnf-automatic`, установим ПО для автоматического обновления пакетов системы:
![Установка ПО для автоматического обновления системы](image/img23.png){#fig:023 width=70%}

Следующим шагом будет настройка конфигурационного файла `/etc/dnf/automatic.conf`. В нем меняем значение поля `apply_update`:
![Изменение конфигурационного файла automatic.conf](image/img24.png){#fig:024 width=70%}

После чего включаем таймер:
![Включение таймера](image/img25.png){#fig:025 width=70%}
...
![Изменение параметров selinux](image/img26.png){#fig:026 width=70%}
...
![Установка Development Tools](image/img27.png){#fig:027 width=70%}

Далее установим `dkms` при помощи `dnf install dkms`:
![Установка dkms](image/img28.png){#fig:028 width=70%}

После чего монтируем диск командой `mount /dev/sr0 /media` и запускаем файл `VBoxLinuxAdditions.run`:
![Установка Guest Additions](image/img29.png){#fig:029 width=70%}
...
![Установка Guest Additions](image/img30.png){#fig:030 width=70%}

После чего просто перезагружаем машину с помощью `reboot`.
## 9. Изменение имени хоста 
![Статическое имя хоста](image/img31.png){#fig:031 width=70%}

## 10. Подключение общей папки 
Для начала добавим своего пользователя в группу vboxsf через `gpasswd -a zdtimurovna vboxsf`:
![Добавление пользователя в группу vboxsf](image/img32.png){#fig:032 width=70%}

Далее в графическом интерфейсе добавим хостовую папку в общую с ВМ:
![Добавление общей папки](image/img33.png){#fig:033 width=70%}

## 11. Работа с языком разметки Markdown 
Установим pandoc:
![Установка pandoc](image/img34.png){#fig:034 width=70%}

## 12. Установка texlive 
Команда для установки `dnf -y install texlive-scheme-full`:
![Установка pandoc](image/img35.png){#fig:035 width=70%}

## 13. Домашнее задание 
Вывод  команды `dmesg | less`:
![Вывод команды dmesg | less](image/img36.png){#fig:036 width=70%}

Версия ядра Linux:
![Получение версии ядра](image/img37.png){#fig:037 width=70%}

Частота процессора:
![Получение частоты процессора](image/img38.png){#fig:038 width=70%}

Модель процессора:
![Получение модели процессора](image/img39.png){#fig:039 width=70%}

Объём доступной оперативной памяти:
![Получение доступной оперативной памяти](image/img40.png){#fig:040 width=70%}

Тип обнаруженного гипервизора:
![Получение гипервизора](image/img41.png){#fig:041 width=70%}

Тип файловой системы корневого раздела и последовательность монтирования файловых систем:
![Тип файловой системы и последовательность монтирования](image/img42.png){#fig:042 width=70%}
